...
function Get-PagesProductionDeployments {
  $raw = (& (Resolve-NpxCliPath) --yes wrangler pages deployment list `
    --project-name $script:PagesProjectName `
    --environment production `
    --json 2>$null | Out-String)

  if ($LASTEXITCODE -ne 0 -or -not $raw.Trim()) {
    throw "No fue posible consultar los despliegues de producción de Cloudflare Pages."
  }

  try {
    $parsed = $raw | ConvertFrom-Json
  } catch {
    throw "Wrangler devolvió una respuesta inválida al consultar los despliegues de producción."
  }
...
function Get-PagesDeploymentBranch {
  param($Deployment)
...
function Resolve-PagesProductionBranch {
  param($Project)

  foreach ($propertyName in @("production_branch", "productionBranch", "Production Branch")) {
  ...
  $deployments = @(Get-PagesProductionDeployments)
  foreach ($deployment in $deployments) {
    $branch = Get-PagesDeploymentBranch $deployment
    if ($branch) {
      return $branch
    }
  }

  # Un proyecto recién creado todavía puede no tener despliegues.
  return $script:ProductionBranch
}
...
function Ensure-PagesProject {
  ...
  $script:PagesProductionBranch = Resolve-PagesProductionBranch $project

  if (-not $script:PagesProductionBranch) {
    throw "No fue posible determinar la rama de producción de Cloudflare Pages."
  }

  Write-Host "Cloudflare Pages: $script:PagesProjectName | producción: $script:PagesProductionBranch"

  if ($script:PagesProductionBranch -ne $script:ProductionBranch) {
    Write-Warning "El repositorio usa '$script:ProductionBranch', pero Cloudflare Pages tiene '$script:PagesProductionBranch' como rama de producción. Se usará la rama de Cloudflare para actualizar el dominio principal."
  }
...
function Deploy-Pages {
  ...
  Invoke-ExternalCommand "Build final de Pages" { npm run build }

  if (-not $script:PagesProductionBranch) {
    Ensure-PagesProject | Out-Null
  }

  # Wrangler clasifica el despliegue como producción cuando la rama coincide con
  # production_branch del proyecto Pages.
  $result = Invoke-WranglerProcess -Arguments @(
    "pages", "deploy", "dist",
    "--project-name", $script:PagesProjectName,
    "--branch", $script:PagesProductionBranch,
    "--commit-dirty=true"
  )
  $result.Output | ForEach-Object { Write-Host $_ }
  ...
  $deploymentMatch = [regex]::Match(
    $outputText,
    "https://[a-z0-9-]+\.$([regex]::Escape($script:PagesProjectName))\.pages\.dev"
  )

  if ($deploymentMatch.Success) {
    $deploymentUrl = $deploymentMatch.Value.TrimEnd("/")
  }

  Verify-Production -DeploymentUrl $deploymentUrl
}
...
function Verify-Production {
  ...
  $versionPath = Join-Path $PSScriptRoot "dist\version.json"
  if (-not (Test-Path -LiteralPath $versionPath)) {
    Invoke-ExternalCommand "Build para verificar producción" { npm run build }
  }
  ...
  if ($DeploymentUrl) {
    $env:HOPPER_DEPLOYMENT_URL = $DeploymentUrl
  }

  try {
    Invoke-ExternalCommand "Verificación exacta de producción" { node scripts/verify-production.mjs }
  } finally {
    ...
    Remove-Item Env:\HOPPER_DEPLOYMENT_URL -ErrorAction SilentlyContinue
  }
}
