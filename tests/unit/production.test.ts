...
describe("configuración final de producción", () => {
  ...
  test("la configuración no contiene secretos ni campos de migración retirados", async () => {
    ...
    expect(source).not.toMatch(/legacyPublicAppUrl|cutoverComplete|legacyRedirectVerified/);
  });

  test("los despliegues frontend y Worker son independientes y usan master", async () => {
    ...
    expect(frontend).toContain("--branch=master");
    ...
    expect(frontend).not.toContain("cutoverComplete");
    expect(worker).not.toContain("cutoverComplete");
  });
});
