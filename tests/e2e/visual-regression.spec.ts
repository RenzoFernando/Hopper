...
async function disableRuntime(page: Page) {
  await page.route("**/js/**", (route) => route.abort());
  await page.route("**/service-worker.js", (route) => route.abort());
}

async function openLegacy(page: Page, path: string, width = 1440, height = 1000) {
  await page.setViewportSize({ width, height });
  await disableRuntime(page);
...
test("inicio móvil conserva el baseline", async ({ page }) => {
  await openLegacy(page, "/index.html", 390, 844);
  // Windows rasteriza este viewport móvil de forma ligeramente distinta; CI Linux conserva el umbral global estricto.
...
test("inicio escritorio conserva el baseline", async ({ page }) => {
  await openLegacy(page, "/index.html", 1440, 1000);
  await expect(page).toHaveScreenshot("home-desktop.png", { fullPage: true });
});

test("Mi espacio conserva estados representativos", async ({ page }) => {
  await openLegacy(page, "/index.html");
...
test("selección y progreso conservan el baseline", async ({ page }) => {
  await openLegacy(page, "/index.html");
  await showWorkspace(page);
  await page.evaluate(() => {
...
test("salas conservan creación, acceso y estado activo", async ({ page }) => {
  await openLegacy(page, "/index.html");
  await page.evaluate(() => {
  ...
  await openLegacy(page, "/room.html#AB-1234");
...
test("administración y cambio de PIN conservan el baseline", async ({ page }) => {
  await openLegacy(page, "/admin.html", 1440, 1200);
...
test("recuperación conserva el baseline", async ({ page }) => {
  await openLegacy(page, "/recover.html#token=baseline", 900, 900);
  await expect(page).toHaveScreenshot("recover.png", { fullPage: true });
});

test("Share Target conserva estados sin sesión y autenticado", async ({ page }) => {
  await openLegacy(page, "/share-target.html", 900, 1000);
  await page.evaluate(() => {
