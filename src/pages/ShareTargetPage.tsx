...
export function ShareTargetPage() {
  ...
  useEffect(() => {
    if (visual) return;
    const notices = [
      { message: share.pinMessage, kind: share.pinKind },
      { message: share.roomMessage, kind: share.roomKind },
      { message: share.message, kind: share.messageKind }
    ];
    notices.forEach((notice, index) => {
      if (!notice.message || !notice.kind || lastNotices.current[index] === notice.message) return;
      lastNotices.current[index] = notice.message;
      showToast(notice.message, notice.kind);
    });
  }, [share.message, share.messageKind, share.pinKind, share.pinMessage, share.roomKind, share.roomMessage, showToast, visual]);
      ...
      <main className="share-main">
        <section className="share-card" aria-labelledby="share-title">
          ...
          <div className="share-auth" id="share-auth" hidden={!showAuth}>
            <p className="share-auth-copy" id="share-auth-copy">{visual ? (share.hasDestination ? "Puedes añadir otro destino antes de enviar." : "Accede a Mi espacio o entra a una sala para elegir el destino.") : (share.hasDestination ? "Puedes usar el destino activo o autenticar otro." : "Completa el PIN, escribe un código o crea una sala. Hopper enviará el contenido automáticamente.")}</p>
            <form className="share-auth-form" id="share-pin-form" noValidate hidden={!showPin} onSubmit={(event) => { event.preventDefault(); void share.submitPin(); }}>
              ...
              <div className={`share-auth-row ${visual ? "" : "is-auto"}`.trim()}><input id="share-pin" type="password" inputMode="numeric" maxLength={4} pattern="[0-9]{4}" autoComplete="current-password" enterKeyHint="go" aria-label="PIN de Mi espacio" placeholder="••••" value={share.pin} disabled={share.authBusy || share.sendBusy} onChange={(event) => share.setPin(event.target.value)} /><button className="secondary-button" id="share-pin-submit" type="submit" hidden={!visual} disabled={share.authBusy || share.sendBusy}>Entrar</button></div>
              {!visual && <span className="share-auto-hint">Se envía al completar los 4 dígitos.</span>}
              <p className={`share-auth-message ${share.pinKind ? `is-${share.pinKind}` : ""}`.trim()} id="share-pin-message" aria-live="polite" hidden={!visual || !share.pinMessage}>{share.pinMessage}</p>
            </form>
            ...
            <form className="share-auth-form" id="share-room-form" noValidate hidden={!showRoom} onSubmit={(event) => { event.preventDefault(); void share.submitRoom(); }}>
              ...
              <div className={`share-auth-row ${visual ? "" : "is-auto"}`.trim()}><RoomCodeInput id="share-room-code" value={share.roomCode} ariaLabel="Código de sala" disabled={share.authBusy || share.sendBusy} onChange={share.setRoomCode} /><button className="secondary-button" id="share-room-submit" type="submit" hidden={!visual} disabled={share.authBusy || share.sendBusy}>Entrar</button></div>
              {!visual && <span className="share-auto-hint">Se entra y envía al completar el código.</span>}
              <p className={`share-auth-message ${share.roomKind ? `is-${share.roomKind}` : ""}`.trim()} id="share-room-message" aria-live="polite" hidden={!visual || !share.roomMessage}>{share.roomMessage}</p>
            </form>
            {!visual && showRoom && <button className="secondary-button share-create-room" id="share-create-room" type="button" disabled={share.authBusy || share.sendBusy || !share.hasContent} onClick={() => { void share.createRoomAndSend(); }}>Crear sala y enviar</button>}
          </div>
          ...
          <p className="share-progress" id="share-progress" aria-live="polite">{share.progress}</p><p className={`form-message ${share.messageKind ? `is-${share.messageKind}` : ""}`.trim()} id="share-message" aria-live="polite" hidden={!visual}>{share.message}</p>
